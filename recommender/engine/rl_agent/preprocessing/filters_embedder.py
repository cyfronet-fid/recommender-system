...
class FiltersEmbedder:
    ...
    def __init__(self, services_transformer=None, services_embedder=None):
        """
        Args:
            services_transformer: Services transformer, by default most recent
             is loaded from the database.
            services_embedder: Services embedder, by default the encoder from
             the most recent Services Autoencoder is loaded from the database.
        """

        if services_transformer is not None:
            self.services_transformer = services_transformer
        else:
            self.services_transformer = load_last_transformer(SERVICES)

        if services_embedder is not None:
            self.services_embedder = services_embedder
        else:
            self.services_embedder = load_last_module(SERVICES_AUTOENCODER).encoder
