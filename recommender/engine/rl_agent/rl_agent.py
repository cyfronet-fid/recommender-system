...
class RLAgentRecommender(BaseAgentRecommender):
    ...
    def __init__(
        ...
        super().__init__()
        self.actor_model_v1 = actor_model_v1
        self.actor_model_v2 = actor_model_v2
    ...
    def _load_models(self) -> None:
        ...
        if self.actor_model_v1 is None:
            try:
                self.actor_model_v1 = load_last_module(
                    ACTOR_MODEL_V1
        ...
        if self.actor_model_v2 is None:
            try:
                self.actor_model_v2 = load_last_module(
                    ACTOR_MODEL_V2
                )  # TODO: import this constant from actor_model.py
            except NoSavedModuleError as no_saved_module:
                raise UntrainedRLAgentError from no_saved_module
    ...
    def _choose_actor_model(self, k: int) -> torch.nn.Module:
        ...
        if k == 2:
            actor_model = self.actor_model_v1
        elif k == 3:
            actor_model = self.actor_model_v2
        else:
