...
from recommender.engine.agents.rl_agent.models.search_phrase_embedder import SearchPhraseEmbedder
from recommender.engine.agents.rl_agent.preprocessing.state_encoder import StateEmbedder
...
def test_filters_embedder(mongo):
    ...
    filters_embedder = FiltersEncoder(
        services_transformer=services_transformer, service_embedder=service_embedder
    )
...
def test_state_embedder(mongo, mocker):
    ...
    search_phrase_embedder_mock = mocker.patch(
        "recommender.engine.rl_agent.preprocessing.searchphrase_embedder.SearchPhraseEncoder.__call__"
    )
    search_phrase_embedder_mock.return_value = torch.stack(
        [torch.rand(SPE) for _ in range(X)]
    )
    search_phrase_embedder = SearchPhraseEmbedder()

    filters_embedder = lambda _: torch.rand(SE)
    filters_embedder_mock = mocker.patch(
        "recommender.engine.rl_agent.preprocessing.filters_embedder.FiltersEncoder.__call__"
    )
